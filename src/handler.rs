...
fn filter_db_recode(note: &NoteModel) -> NoteModelResponse {
    NoteModelResponse {
        id: note.id.to_owned(),
...
pub async fn note_list_handler(
    ...
    let note_responses = notes
        .into_iter()
        .map(|note| filter_db_recode(&note))
...
async fn edit_note_handler(
    ...
    let update_result = sqlx::query(
        ...
        .bind(body.content.to_owned().unwrap_or_else(|| note.content.clone()))
        .bind(body.category.to_owned().unwrap_or_else(|| note.category.clone().unwrap()))
        .bind(i8_published)
    ...
    let update_note_result = sqlx::query_as!(
        NoteModel,
        r#"SELECT * FROM notes WHERE id = ?"#,
        note_id.to_owned()
    )
    ...
    return match update_note_result {
        Ok(note) => {
